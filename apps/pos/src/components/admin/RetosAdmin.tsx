...
export function RetosAdmin() {
  ...
  const [f, setF] = useState({ nombre: '', tipo: 'visitas', objetivo: '5', periodoDias: '7', recompensaPuntos: '50', insignia: '☕🔥' });

  const crear = async () => {
    if (!f.nombre.trim()) return show('Falta nombre', false);
    await api.createReto({ localId, nombre: f.nombre.trim(), tipo: f.tipo, objetivo: Number(f.objetivo), periodoDias: Number(f.periodoDias), recompensaPuntos: Number(f.recompensaPuntos), insignia: f.insignia });
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <Panel title="Nuevo reto">
        <div className="grid gap-2.5">
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <Field label="Tipo"><Select value={f.tipo} onChange={(e) => setF({ ...f, tipo: e.target.value })}><option value="visitas">Visitas</option><option value="gasto">Gasto</option></Select></Field>
            <Field label="Objetivo"><TextInput inputMode="decimal" value={f.objetivo} onChange={(e) => setF({ ...f, objetivo: e.target.value })} /></Field>
          </div>
          <div className="grid grid-cols-3 gap-2.5">
            <Field label="Días"><TextInput inputMode="numeric" value={f.periodoDias} onChange={(e) => setF({ ...f, periodoDias: e.target.value })} /></Field>
      ...
      <Panel title="Retos">
        {retos.isLoading ? <Empty>Cargando…</Empty> : retos.data?.length === 0 ? <Empty>Sin retos</Empty> : (
          <div className="space-y-2">
            {(retos.data ?? []).map((r) => (
              <div key={r.id} className="flex items-center justify-between rounded-lg border border-line px-3 py-2 text-sm">
                ...
                <span className="text-xs text-muted">{r.tipo} · <span className="font-mono tnum">{Number(r.objetivo)}</span></span>
              </div>
            ))}
          </div>
        )}
