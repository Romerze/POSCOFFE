...
export function CatalogAdmin() {
  ...
  const [receta, setReceta] = useState<{ id: string; nombre: string } | null>(null);

  const refresh = () => { qc.invalidateQueries({ queryKey: ['adm-prods', localId] }); qc.invalidateQueries({ queryKey: ['productos', localId] }); };
  ...
  const addProd = async () => {
    if (!prod.nombre.trim() || !prod.categoriaId) return show('Falta nombre o categoría', false);
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <Panel title="Categorías">
        ...
        <div className="flex flex-wrap gap-2">
          {(categorias.data ?? []).map((c) => <span key={c.id} className="rounded-full bg-surface2 px-3 py-1 text-sm text-fg">{c.nombre}</span>)}
      ...
      <Panel title="Nuevo producto">
        <div className="grid gap-2.5">
          ...
          <label className="flex items-center gap-2 text-sm text-fg"><input type="checkbox" checked={prod.esCombo} onChange={(e) => setProd({ ...prod, esCombo: e.target.checked })} /> Es combo</label>
      ...
      <div className="lg:col-span-2">
        <Panel title="Productos y variantes">
          {productos.isLoading ? <Empty>Cargando…</Empty> : productos.data?.length === 0 ? <Empty>Aún no hay productos</Empty> : (
            <div className="space-y-3">
              {(productos.data ?? []).map((p) => <Row key={p.id} producto={p} onChange={refresh} onReceta={setReceta} onToast={show} />)}
            </div>
          )}
        </Panel>
      </div>

      {receta && <RecetaEditor variante={receta} onClose={() => setReceta(null)} onSaved={() => { setReceta(null); refresh(); show('Receta guardada'); }} />}
      {node}
    </div>
  );
}

function Row({ producto, onChange, onReceta, onToast }: { producto: Producto; onChange: () => void; onReceta: (v: { id: string; nombre: string }) => void; onToast: (m: string, ok?: boolean) => void }) {
  const [nombre, setNombre] = useState('');
  const [precio, setPrecio] = useState('');
  const add = async () => {
    if (!nombre.trim() || !precio) return onToast('Falta nombre o precio', false);
    await api.createVariante({ productoId: producto.id, nombre: nombre.trim(), precio: Number(precio) });
    setNombre(''); setPrecio(''); onChange(); onToast('Variante añadida');
  };
  return (
    <div className="rounded-lg border border-line p-3">
      <div className="mb-2 flex items-center gap-2">
        <span className="font-display font-bold text-fg">{producto.nombre}</span>
      ...
      <div className="flex flex-wrap gap-2">
        {producto.variantes.map((v) => (
          <button key={v.id} onClick={() => onReceta({ id: v.id, nombre: `${producto.nombre} ${v.nombre}` })} className="rounded-lg bg-surface2 px-3 py-1.5 text-sm text-fg transition hover:brightness-95" title="Editar receta">
            {v.nombre} · <span className="font-mono tnum text-cherry">S/{Number(v.precio).toFixed(2)}</span>
          </button>
        ))}
      </div>
      <div className="mt-2 flex gap-2">
        <TextInput placeholder="Variante (ej. Grande)" value={nombre} onChange={(e) => setNombre(e.target.value)} />
        <TextInput placeholder="Precio" inputMode="decimal" value={precio} onChange={(e) => setPrecio(e.target.value)} className="max-w-[7rem]" />
        <Btn tone="ghost" onClick={add}>+ Variante</Btn>
      </div>
    </div>
  );
}
