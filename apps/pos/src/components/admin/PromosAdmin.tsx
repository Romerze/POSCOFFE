...
export function PromosAdmin() {
  ...
  const [f, setF] = useState({ nombre: '', tipo: 'descuento', efectoTipo: 'porcentaje', valor: '', n: '2', m: '1', prioridad: '1', montoMinimo: '', franjaDesde: '', franjaHasta: '', vigenciaDesde: '2026-01-01', vigenciaHasta: '2026-12-31' });

  const crear = async () => {
    if (!f.nombre.trim()) return show('Falta nombre', false);
    ...
    const efecto = f.tipo === 'nxm' ? { tipo: 'nxm', n: Number(f.n), m: Number(f.m) } : { tipo: f.efectoTipo, valor: Number(f.valor) };
    await api.createPromo({ localId, nombre: f.nombre.trim(), tipo: f.tipo, vigenciaDesde: f.vigenciaDesde, vigenciaHasta: f.vigenciaHasta, prioridad: Number(f.prioridad), condicion, efecto });
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <Panel title="Nueva promoción">
        <div className="grid gap-2.5">
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <Field label="Tipo"><Select value={f.tipo} onChange={(e) => setF({ ...f, tipo: e.target.value })}><option value="descuento">Descuento</option><option value="hora_valle">Hora valle</option><option value="nxm">NxM</option></Select></Field>
          ...
          {f.tipo === 'nxm' ? (
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <Field label="Efecto"><Select value={f.efectoTipo} onChange={(e) => setF({ ...f, efectoTipo: e.target.value })}><option value="porcentaje">Porcentaje %</option><option value="monto">Monto S/</option></Select></Field>
              <Field label="Valor"><TextInput inputMode="decimal" value={f.valor} onChange={(e) => setF({ ...f, valor: e.target.value })} /></Field>
            </div>
          )}
          <div className="grid grid-cols-2 gap-2.5">
            <Field label="Franja desde"><TextInput placeholder="15:00" value={f.franjaDesde} onChange={(e) => setF({ ...f, franjaDesde: e.target.value })} /></Field>
            <Field label="Franja hasta"><TextInput placeholder="18:00" value={f.franjaHasta} onChange={(e) => setF({ ...f, franjaHasta: e.target.value })} /></Field>
          </div>
      ...
      <Panel title="Promociones activas">
        {promos.isLoading ? <Empty>Cargando…</Empty> : promos.data?.length === 0 ? <Empty>Sin promociones</Empty> : (
            ...
            {(promos.data ?? []).map((p) => (
              <div key={p.id} className="flex items-center justify-between rounded-lg border border-line px-3 py-2 text-sm">
                ...
                <span className="flex items-center gap-2 text-xs text-muted">
                  ...
                  <span className={p.activa ? 'text-pine' : 'text-danger'}>●</span>
                </span>
              </div>
            ))}
          </div>
